...
def get_movenet_data(path):
    # path1 = kagglehub.model_download("google/movenet/tfLite/singlepose-thunder")
    # print(path1)
    # estimator = tf.lite.Interpreter(model_path=path1)
    # estimator.allocate_tensors()
    MODEL_PATH = '3.tflite'
    estimator = tf.lite.Interpreter(model_path=MODEL_PATH)
    estimator.allocate_tensors()
    ...
    while cap.isOpened():
        ...
        if ret:
            ...
            vid_keypts.append(np.squeeze(keypts))
