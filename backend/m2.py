...
import numpy as np
from frame_selector import get_movenet_data, frame_with_max_change, processed
...
def create_ssbd_model2():
    ...
    joint_input = tf.keras.Input(shape = (None, 34,))
...
def m2_identify(model, video_path):
    ...
    print(np.array(best_frame).shape, np.array(keypts).shape)
    keypts = processed(keypts)
    keypts = np.expand_dims(keypts, axis=0)
